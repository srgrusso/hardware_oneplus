...
public class HeadphoneGainPreference extends CustomSeekBarPreference {
    ...
    private static final int NODE_LEVEL = R.string.node_headphone_gain_preference;

    public HeadphoneGainPreference(Context context, AttributeSet attrs) {
        super(context, attrs);

        mInterval = context.getResources().getInteger(R.integer.headphone_gain_preference_interval);
        mShowSign = false;
        mUnits = "";
        mContinuousUpdates = false;
        int[] mAllValues = context.getResources().getIntArray(R.array.headphone_gain_preference_array);
    ...
    public static void restore(Context context) {
        ...
        int[] mAllValues = context.getResources().getIntArray(R.array.headphone_gain_preference_array);
        mDefVal = mAllValues[0];
        String storedValue = PreferenceManager.getDefaultSharedPreferences(context).getString(DeviceExtras.KEY_HEADPHONE_GAIN, String.valueOf(mDefVal));
    ...
    private void saveValue(String newValue) {
        ...
        editor.putString(DeviceExtras.KEY_HEADPHONE_GAIN, newValue);
        editor.apply();
    }
